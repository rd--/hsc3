...
buffer - the buffer LPC data is stored (see LPC.read)
ptr - index into LPC data, 0 is the begining and 1 is the end.
excitation - the signal to filter.

This LPC data files read are those generated by lpanal (see csound).

Note: since the LPC data set will likely exceed the UDP packet 
limit for, load_data splits the LPC data into 512*4 byte packets.
...
>       in s * 1e-5 * rms }
> in withSC3 (\fd -> do { lpc <- lpcRead "/home/rohan/tn/tn-56/lpc/fate.lpc"
>                       ; n <- pinkNoise AR
>                       ; let { d = lpcSC3 lpc 

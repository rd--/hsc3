...
let n = 6
    ...
    f = X.rRandNId n 'η' base (base + rng)
    dt = X.rRandNId n 'θ' 0.1 2
...
let f (c,g,x,y,z,_,_,_,_,_,_) =
      let n = 6
          ...
          n1 = lfNoise1 kr r0
          fq = X.rtRandN n base (base + rng) g
          dt = X.rtRandN n 0.1 2 g
...
let f (c,g,x,y,z,_,_,_,_,_,_) =
      let n = 6
          ...
          n1 = lfNoise1Id 'ζ' kr r0
          fq = X.rtRandNId n 'η' base (base + rng) g
          dt = X.rtRandNId n 'θ' 0.1 2 g
          exc = n0 * 0.01 * z * lagUD g y 2
          k = klankSpec_mce fq (mce (replicate n 1)) dt
          s = dynKlank exc 1 0 1 k
      in softClip (dynKlank exc 1 0 1 k)

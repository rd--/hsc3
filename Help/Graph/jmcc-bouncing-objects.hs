...
let imp_frq = xLine kr (5 + rand (-2) 2) 600 4 DoNothing
    ...
    exc = decay imp 0.001
    flt_frq = X.rRandN 4 400 8400
    flt_amp = X.rRandN 4 0 1
    flt_rtm = X.rRandN 4 0.01 0.11
...
let imp_frq = xLine kr (5 + randId 'α' (-2) 2) 600 4 DoNothing
    ...
    imp = impulse ar imp_frq 0 * imp_amp
    exc = decay imp 0.001
    flt_frq = X.rRandNId 4 'β' 400 8400
    flt_amp = X.rRandNId 4 'γ' 0 1
    flt_rtm = X.rRandNId 4 'δ' 0.01 0.11
    flt = klank exc 1 0 1 (klankSpec_mce flt_frq flt_amp flt_rtm)

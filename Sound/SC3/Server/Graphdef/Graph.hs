...
make_input :: Graph.Maps -> Graph.From_Port -> Graphdef.Input
make_input (cs,ks,_,us,kt) fp =
    case fp of
      Graph.From_Port_C n -> Graphdef.Input (-1) (Graph.fetch n cs)
      Graph.From_Port_K n t ->
        let i = Graph.ktype_map_lookup t kt
        in Graphdef.Input i (Graph.fetch_k n t ks)
      Graph.From_Port_U n p -> Graphdef.Input (Graph.fetch n us) (fromMaybe 0 p)

node_k_to_control :: Graph.Maps -> Graph.Node -> Graphdef.Control
node_k_to_control (_,_,ks,_,_) nd =
    case nd of
      Graph.Node_K n _ _ nm _ _ _ -> (Datum.ascii nm,Graph.fetch n ks)
      _ -> error "node_k_to_control"

-- | Byte-encode 'Node_U' primitive node.
node_u_to_ugen :: Graph.Maps -> Graph.Node -> Graphdef.UGen
node_u_to_ugen m n =
    case n of
      Graph.Node_U _ r nm i o (Type.Special s) _ ->

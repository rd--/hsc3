...
mk_node_k :: Graphdef -> G.Node_Id -> (Control,U.Sample) -> G.Node
mk_node_k g z ((nm,ix),v) =
    let z' = graphdef_control_nid g z
        nm' = ascii_to_string nm
    in G.Node_K z' R.KR (Just ix) nm' v G.K_KR Nothing

input_to_from_port :: Graphdef -> Input -> G.From_Port
input_to_from_port g (Input u p) =
    if u == -1
    then G.From_Port_C (graphdef_constant_nid g p)
    else if input_is_control g (Input u p)
         then if u /= 0
              then error "multiple control UGens..."
              else G.From_Port_K (graphdef_control_nid g p) G.K_KR
         else let ugen = graphdef_ugens g !! u
                  port = if length (ugen_outputs ugen) > 1
                         then Just p
                         else Nothing
              in G.From_Port_U (graphdef_ugen_nid g u) port

mk_node_u :: Graphdef -> G.Node_Id -> UGen -> G.Node
mk_node_u g z u =
    ...
    in G.Node_U z' rate' name' inputs' outputs' special' (U.UId z')

graphdef_to_graph :: Graphdef -> (String,G.Graph)
graphdef_to_graph g =
    let constants_nd = zipWith G.Node_C [0..] (graphdef_constants g)
